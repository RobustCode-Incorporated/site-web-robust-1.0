...
- **Localized product pages**: **now live**, using the same `.fr.md` companion-file mechanism as Insights articles (see `loadProducts(lang)` in `scripts/build-content.mjs`) — `fr/store/` output tree, hreflang tags, translated chrome/labels via `PT.fr` and `PRODUCT_CATEGORIES.fr`. The USB-C cable (`content/products/tech/usb-c-to-usb-c-cable.fr.md`) is the first example. Future products need their own `.fr.md` file the same way.
